...
from modules.mercosul_engine.mercosul_engine import MercosulEngine
from modules.mercosul_engine.relatorio_engine_mc import RelatorioMercosul

st.set_page_config(page_title="Mercosul", layout="wide")
...
PASTA_PROCESSADOS = RAIZ_PROJETO / "processados" / "mercosul"
PASTA_PROCESSADOS.mkdir(parents=True, exist_ok=True)

st.markdown('<div class="main-header"><h1>⚓ Auditoria Mercosul</h1><h3>Processamento de CTEs</h3></div>', unsafe_allow_html=True)

if 'resultados_mercosul' not in st.session_state:
    st.session_state['resultados_mercosul'] = None
if 'relatorio_mercosul' not in st.session_state:
    st.session_state['relatorio_mercosul'] = None
        ...
        if st.button("🔍 PROCESSAR ARQUIVOS", use_container_width=True, type="primary"):
            with st.spinner("Processando..."):
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    engine = MercosulEngine()
                    resultados = engine.processar_arquivos(temp_path, pasta_destino)
                    
                    st.session_state['resultados_mercosul'] = resultados
                ...
                with st.spinner("Gerando relatório..."):
                    ...
                    if not xmls_encontrados:
                        ...
                        with tempfile.TemporaryDirectory() as temp_dir:
                            ...
                            relatorio = RelatorioMercosul()
                            pasta_relatorio = PASTA_PROCESSADOS / "relatorios"
                            resultado = relatorio.gerar_relatorio(temp_path, pasta_relatorio)
                            
                            if resultado:
                                st.session_state['relatorio_mercosul'] = resultado
                                st.success(f"✅ Relatório com {resultado['total_ctes']} CTEs!")
                                st.balloons()
        else:
            st.info("Processe os arquivos primeiro")

if st.session_state['resultados_mercosul']:
    st.divider()
    st.subheader("📊 Resultados")
    
    resultados = st.session_state['resultados_mercosul']
    stats = MercosulEngine().obter_estatisticas(resultados)
...
if st.session_state['relatorio_mercosul']:
    st.divider()
    resultado = st.session_state['relatorio_mercosul']
...
st.caption("⚓ Auditoria Mercosul v1.0")