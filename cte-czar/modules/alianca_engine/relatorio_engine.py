...
class RelatorioAlianca:
    ...
    def _extrair_info_toma4(self, root):
        try:
            for elem in root.iter():
                if '}' in elem.tag:
                    tag = elem.tag.split('}')[-1]
            ...
            print(f"Erro ao extrair info do toma4: {e}")
        return None
    ...
    def extrair_informacoes_xml(self, xml_path):
        try:
            ...
            if xObs:
                match = re.search(r'Lacre:\s*(\w{4}\d{7})', xObs)
                if match:
                    container = match.group(1)
                else:
                    match = re.search(r'(\w{4}\d{7})', xObs)
                    if match:
                        container = match.group(1)
            
            info_toma4 = self._extrair_info_toma4(root)
            cnpj_completo = info_toma4['cnpj'] if info_toma4 else None
            codigo = info_toma4['codigo'] if info_toma4 else None
            filial = info_toma4['filial'] if info_toma4 else None
            ...
            frete_bruto = self._extrair_valor_componente(root, 'FRETE VALOR')
            infraestrutura = self._extrair_valor_componente(root, 'INFRAESTRUTURA')
            
            frete_liquido = 0.0
            vBC = self._extrair_texto_tag(root, 'vBC')
            if vBC:
                try:
                    frete_liquido = float(vBC.replace(',', '.'))
                except:
                    pass
    ...
    def gerar_relatorio(self, pasta_origem, pasta_destino=None):
        try:
            ...
            for container, infos in ctes_por_container.items():
                ...
                total_infra = sum(info.get('infraestrutura', 0.0) for info in infos)
                ...
                linha = {
                    ...
                    'Infraestrutura (Total)': round(total_infra, 2),
                }
                linhas.append(linha)
            ...
            for info in sem_container:
                ...
                linha = {
                    ...
                    'Infraestrutura (Total)': round(info.get('infraestrutura', 0.0), 2),
                }
                linhas.append(linha)
            ...
            nome_arquivo = f"relatorio_alianca_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
