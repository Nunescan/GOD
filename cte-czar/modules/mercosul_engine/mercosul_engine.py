        ...
        for arquivo in arquivos:
            ...
            if extensao == '.pdf':
                ...
                if numero:
                    ...
                    # Extrai proposta da tag xObs
                    try:
                        tree = ET.parse(arquivo)
                        root = tree.getroot()
                        for elem in root.iter():
                            if 'xObs' in elem.tag and elem.text:
                                match = re.search(r'PC:\d+/(\d+)', elem.text)
                                if match:
                                    cte_map[numero]['proposta'] = match.group(1)
                    except:
                        pass
