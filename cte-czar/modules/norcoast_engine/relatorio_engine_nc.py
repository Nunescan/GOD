...
class RelatorioNorcoast:
    ...
    def extrair_informacoes_xml(self, xml_path):
        try:
            ...
            bunker = self._extrair_valor_componente(root, 'BUNKER')
            frete_bruto = self._extrair_valor_componente(root, 'FRETE')
            
            # Frete Líquido (vTPrest)
            frete_liquido = 0.0
    ...
    def gerar_relatorio(self, pasta_origem, pasta_destino=None):
        try:
            ...
            for container, infos in ctes_por_container.items():
                ...
                total_bunker = sum(info.get('bunker', 0.0) for info in infos)
                ...
                linha = {
                    ...
                    'Bunker (Total)': round(total_bunker, 2),
            ...
            for info in sem_container:
                ...
                linha = {
                    ...
                    'Bunker (Total)': round(info.get('bunker', 0.0), 2),
            ...
            nome_arquivo = f"relatorio_norcoast_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
